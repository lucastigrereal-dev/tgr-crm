...
O ledger possui tabelas append-only, fingerprint idempotente, versão de política, API de leitura e card na ficha contratual. A projeção já é provada por contrato, parcela, comissão e distrato. Ela agora é acionada automaticamente após criação/alteração de status do contrato, execução de distrato e baixa de parcela — esta última também cobre a comissão de entrada criada nesse mesmo fluxo. A sincronização explícita autorizada continua disponível na ficha. O lançamento ou a mudança de status de comissão manual sem contrato-fonte segue como evolução pendente até ganhar vínculo contratual e evento próprio; não será chamado de automação completa antes disso.
