...
export default function SalesRoom() {
  ...
  return <div className="space-y-7">
    <PageHeader eyebrow="Ritmo da operação" title="Sala de vendas" description="A recepção enxerga a fila do dia, confirma chegada, entrega mesa, time e gerência, mede o tour e registra sem-tour sem buraco na história." />
...
function MetricCard({ icon, label, value, detail, dark, tone }: { icon: React.ReactNode; label: string; value: number; detail: string; dark?: boolean; tone?: "green" | "purple" }) {
  ...
  return <Card className={className}><CardContent className="p-5">{icon}<p className={`mt-5 text-xs font-bold uppercase tracking-[.14em] ${dark ? "text-white/55" : "text-[#52615c]"}`}>{label}</p><p className="mt-1 font-serif text-4xl text-[#1d2b2a]">{value}</p><p className={`mt-1 text-sm ${dark ? "text-white/65" : "text-[#52615c]"}`}>{detail}</p></CardContent></Card>;
}
