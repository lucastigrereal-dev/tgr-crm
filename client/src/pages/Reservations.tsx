...
function InventoryDialog() {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl"><DialogHeader><DialogTitle className="font-serif text-2xl">Empreendimentos & unidades</DialogTitle></DialogHeader>
      {resorts.data?.truncated || units.data?.truncated ? <p className="mb-4 rounded-xl border border-[#ead8ad] bg-[#fff9e9] p-3 text-xs text-[#71531a]">Atenção: inventário limitado por {[...(resorts.data?.truncatedSources ?? []), ...(units.data?.truncatedSources ?? [])].join(", ")}. Exibindo apenas o recorte carregado.</p> : null}
...
function OwnershipOperationsDialog() {
  ...
  const eligibleUnits = units.data?.rows?.filter(item => resortId === "none" || item.unit.resortId === Number(resortId)) ?? [];
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl"><DialogHeader><DialogTitle className="font-serif text-2xl">Direitos de uso & integridade do inventário</DialogTitle></DialogHeader>
      ...
      <div className="grid gap-8 py-2 lg:grid-cols-2">
        ...
        <form className="grid gap-3 rounded-2xl border border-[#e8e3d9] bg-[#faf8f3] p-5" onSubmit={submitMaintenance}>
          ...
          <Select value={maintenanceUnitId} onValueChange={setMaintenanceUnitId}><SelectTrigger><SelectValue placeholder="Unidade" /></SelectTrigger><SelectContent>{units.data?.rows?.map(({ unit, resortName }) => <SelectItem key={unit.id} value={String(unit.id)}>{resortName} · {unit.code}</SelectItem>)}</SelectContent></Select>
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle className="font-serif text-2xl">Nova reserva</DialogTitle></DialogHeader>
      <form className="grid gap-4 py-2" onSubmit={submit}>
        <div className="grid gap-2"><Label>Associado *</Label><Select value={customerId} onValueChange={setCustomerId}><SelectTrigger><SelectValue placeholder="Selecione um cliente" /></SelectTrigger><SelectContent>{customers.data?.rows?.map(customer => <SelectItem key={customer.id} value={String(customer.id)}>{customer.fullName}</SelectItem>)}</SelectContent></Select></div>
        ...
        <div className="grid gap-2"><Label>Unidade *</Label><Select value={unitId} onValueChange={setUnitId}><SelectTrigger><SelectValue placeholder="Selecione a unidade" /></SelectTrigger><SelectContent>{units.data?.rows?.filter(item => item.unit.status === "active").map(({ unit, resortName }) => <SelectItem key={unit.id} value={String(unit.id)}>{resortName} · {unit.code}</SelectItem>)}</SelectContent></Select></div>
...
function GuestsDialog({ reservationId, reservationStatus }: { reservationId: number; reservationStatus: "pending" | "confirmed" | "checked_in" | "completed" | "cancelled" }) {
  ...
  const guestRows = guests.data?.rows ?? [];
