...
export function suggestCsvMapping(content: string, kind: ImportKind) {
  const raw = parseRawTable(content); const fields = kind === "customers" ? customerFields : contractFields;
  ...
  return { sourceHeaders: raw.headers, suggestedMapping, targetFields: fields, requiredFields: kind === "customers" ? ["nome_completo", "documento"] : ["numero_contrato", "documento_associado", "modelo_uso", "valor_total", "quantidade_parcelas", "primeiro_vencimento"] };
}
