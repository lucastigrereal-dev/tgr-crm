...
describe("integridade de referências de cobrança", () => {
  ...
  it("registra cobrança manual com referência normalizada quando livre", async () => {
    ...
    expect(dbMocks.recordDomainEvent).toHaveBeenCalledWith(expect.objectContaining({ eventName: "financial.billing.created", aggregateType: "billing_record", aggregateId: 701, payload: { installmentId: 91, gatewayProvider: "manual", gatewayPaymentId: null, type: "card" } }));
  });
});
