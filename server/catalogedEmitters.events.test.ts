...
function makeDb() {
  ...
  const tx = {
    select: vi.fn(() => ({ from: (table: unknown) => table === unitMaintenanceBlocks || table === installmentRenegotiations || table === proposals ? queryRows([]) : table === opportunities ? queryRows([{ id: 301, stage: "qualified" }]) : table === installments ? queryRows([installment]) : queryRows([{ id: 51 }]) })),
