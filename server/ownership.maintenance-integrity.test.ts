...
import { reservations, unitMaintenanceBlocks, units } from "../drizzle/schema";
...
function makeDb(unitRows: unknown[], conflicts: unknown[], reservationConflicts: unknown[] = []) {
  const inserted: unknown[] = [];
  const limitCalls: number[] = [];
  const tx = {
    select: vi.fn(() => ({ from: (table: unknown) => table === units ? query(unitRows, limitCalls) : table === unitMaintenanceBlocks ? query(conflicts, limitCalls) : table === reservations ? query(reservationConflicts, limitCalls) : query([], limitCalls) })),
...
describe("integridade de bloqueio de manutenção", () => {
  ...
  it("rejeita bloqueio sobre reserva ativa sem inserir nem auditar", async () => {
    const fixture = makeDb([{ id: 51 }], [], [{ id: 801 }]);
    dbMocks.getDb.mockResolvedValue(fixture.db);
    const caller = ownershipRouter.createCaller({ user: { id: 72, role: "service" } } as never);

    await expect(caller.createMaintenanceBlock({ unitId: 51, startsAt: "2026-10-10", endsAt: "2026-10-12", reason: "Manutenção preventiva" })).rejects.toMatchObject({ code: "CONFLICT", message: expect.stringContaining("reserva") });
    expect(fixture.inserted).toEqual([]);
    expect(fixture.limitCalls).toEqual([1, 1, 1]);
    expect(dbMocks.recordAudit).not.toHaveBeenCalled();
  });
