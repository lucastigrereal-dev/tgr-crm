...
function database() {
  ...
  const select = vi.fn()
    .mockReturnValueOnce({ from: () => ({ leftJoin: () => ({ where: () => chain(captureRows) }) }) })
    .mockReturnValueOnce({ from: () => chain([{ id: 8, name: "Campanha Verão" }]) })
    .mockReturnValueOnce({ from: () => chain([{ id: 10, name: "Paulo Promotor", email: null }, { id: 11, name: "Lia Liner", email: null }, { id: 12, name: "Fábio Fechador", email: null }]) })
    .mockReturnValueOnce({ from: () => ({ where: () => chain([{ id: 3, name: "Resort Teste" }]) }) });
  return { select };
}
