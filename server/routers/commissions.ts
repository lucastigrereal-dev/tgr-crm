...
export const commissionsRouter = router({
  ...
  createCampaign: adminProcedure.input(campaignInput).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco indisponível." }); const existing = (await db.select({ id: salesCampaigns.id }).from(salesCampaigns).where(eq(salesCampaigns.code, input.code)).limit(1))[0]; if (existing) throw new TRPCError({ code: "CONFLICT", message: "Já existe uma campanha com este código." }); const created = await db.insert(salesCampaigns).values({ ...input, startsAt: input.startsAt ? new Date(`${input.startsAt}T12:00:00Z`) : null, endsAt: input.endsAt ? new Date(`${input.endsAt}T12:00:00Z`) : null, commissionRate: input.commissionRate.toFixed(2), targetAmount: input.targetAmount.toFixed(2) }).$returningId(); const id = created[0]?.id; if (!id) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Não foi possível criar a campanha." }); await recordAudit(ctx.user.id, "sales_campaign", id, "created", `Campanha ${input.code} criada com meta de R$ ${input.targetAmount.toFixed(2)} e comissão de ${input.commissionRate}%.`); return { id }; }),
  record: financeProcedure.input(z.object({ sellerId: z.number().int().positive(), campaignId: z.number().int().positive().optional(), opportunityId: z.number().int().positive().optional(), contractId: z.number().int().positive().optional(), baseAmount: z.coerce.number().positive(), rate: z.coerce.number().min(0).max(100), notes: z.string().max(2000).optional() })).mutation(async ({ ctx, input }) => { assertCapability(ctx.user.role, "commission.pay"); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco indisponível." }); const [sellerRows, campaignRows, opportunityRows, contractRows] = await Promise.all([db.select({ id: users.id }).from(users).where(eq(users.id, input.sellerId)).limit(1), input.campaignId ? db.select({ id: salesCampaigns.id }).from(salesCampaigns).where(eq(salesCampaigns.id, input.campaignId)).limit(1) : Promise.resolve([]), input.opportunityId ? db.select({ id: opportunities.id }).from(opportunities).where(eq(opportunities.id, input.opportunityId)).limit(1) : Promise.resolve([]), input.contractId ? db.select({ id: contracts.id }).from(contracts).where(eq(contracts.id, input.contractId)).limit(1) : Promise.resolve([])]); if (!sellerRows[0]) throw new TRPCError({ code: "NOT_FOUND", message: "Vendedor da comissão não encontrado." }); if (input.campaignId && !campaignRows[0]) throw new TRPCError({ code: "NOT_FOUND", message: "Campanha da comissão não encontrada." }); if (input.opportunityId && !opportunityRows[0]) throw new TRPCError({ code: "NOT_FOUND", message: "Oportunidade da comissão não encontrada." }); if (input.contractId && !contractRows[0]) throw new TRPCError({ code: "NOT_FOUND", message: "Contrato da comissão não encontrado." }); const amount = calculateCommission(input.baseAmount, input.rate); const created = await db.insert(salesCommissions).values({ sellerId: input.sellerId, campaignId: input.campaignId ?? null, opportunityId: input.opportunityId ?? null, contractId: input.contractId ?? null, baseAmount: input.baseAmount.toFixed(2), rate: input.rate.toFixed(2), amount: amount.toFixed(2), notes: input.notes ?? null }).$returningId(); const id = created[0]?.id; if (!id) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Não foi possível registrar a comissão." }); await recordAudit(ctx.user.id, "sales_commission", id, "created", `Comissão de ${amount.toFixed(2)} lançada.`); await recordDomainEvent({ eventName: "commission.created", aggregateType: "sales_commission", aggregateId: id, actorUserId: ctx.user.id, payload: { sellerId: input.sellerId, campaignId: input.campaignId ?? null, opportunityId: input.opportunityId ?? null, contractId: input.contractId ?? null, amount, rate: input.rate } }); if (input.contractId) await syncRevenueQualityForContract({ contractId: input.contractId, actorUserId: ctx.user.id, trigger: "lançamento manual de comissão vinculada" }); return { id, amount }; }),
  setStatus: financeProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["approved", "paid", "cancelled"]) })).mutation(async ({ ctx, input }) => { assertCapability(ctx.user.role, "commission.pay"); const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco indisponível." }); const commission = (await db.select({ contractId: salesCommissions.contractId, status: salesCommissions.status }).from(salesCommissions).where(eq(salesCommissions.id, input.id)).limit(1))[0]; if (!commission) throw new TRPCError({ code: "NOT_FOUND", message: "Comissão não encontrada." }); if (commission.status === input.status) return { success: true }; if ((commission.status === "paid" || commission.status === "cancelled") && input.status !== commission.status) throw new TRPCError({ code: "CONFLICT", message: `Comissão ${commission.status} não pode voltar para ${input.status}.` }); const updateResult = await db.update(salesCommissions).set({ status: input.status, approvedAt: input.status === "approved" ? new Date() : undefined, paidAt: input.status === "paid" ? new Date() : undefined }).where(and(eq(salesCommissions.id, input.id), eq(salesCommissions.status, commission.status))); if (updateResult && typeof updateResult === "object" && "affectedRows" in updateResult && Number(updateResult.affectedRows) === 0) throw new TRPCError({ code: "CONFLICT", message: "A comissão foi alterada por outra operação. Recarregue e tente novamente." }); await recordAudit(ctx.user.id, "sales_commission", input.id, input.status, `Comissão marcada como ${input.status}.`); await recordDomainEvent({ eventName: "commission.status.updated", aggregateType: "sales_commission", aggregateId: input.id, actorUserId: ctx.user.id, payload: { status: input.status, contractId: commission.contractId } }); if (commission.contractId) await syncRevenueQualityForContract({ contractId: commission.contractId, actorUserId: ctx.user.id, trigger: `status de comissão manual: ${input.status}` }); return { success: true }; }),
});
